...
abstract class TaskManagerTest<T extends TaskManager> {
    ...
    public void updateTasks() {
        Task taskToAdd = new Task("Задача для обновления", "", TaskStatus.NEW);
        taskManager.addTask(taskToAdd);

        Task taskToUpdate = new Task("Задача обновилась", "", TaskStatus.IN_PROGRESS);
        taskManager.updateTask(taskToAdd.getId(), taskToUpdate);
    ...
    public void deleteTasks() {
        Task taskToAdd = new Task("Задача для удаления", "", TaskStatus.NEW);
        Task taskToAdd1 = new Task("Задача для удаления № 2", "", TaskStatus.IN_PROGRESS);
        taskManager.addTask(taskToAdd);
    ...
    public void addNewEpic() {
        ...
        Subtask subtask2 = new Subtask("Вторая подзадача", TaskStatus.NEW);
        ArrayList<Subtask> subtasks = new ArrayList<>();
        subtasks.add(subtask1);
        subtasks.add(subtask2);

        Epic epic1 = new Epic("Первый эпик", "Мой первый эпик");
        taskManager.addEpic(epic1);
        taskManager.addManySubtasks(epic1.getId(), subtasks);
    ...
    public void updateEpics() {
        ...
        Epic epic3 = new Epic("Первый эпик (Upd)", "Тест");
        taskManager.updateEpic(epic1.getId(), epic3);

        final Epic epic = taskManager.getEpicById(epic1.getId());

        assertNotEquals(epic.getTitle(), "Первый эпик", "Названия эпиков совпадают");
        assertNotEquals(epic.getDescription(), "Мой первый эпик", "Описания эпиков совпадают");
        assertEquals(epic.getSubtasks(), subtasks, "Подзадачи не совпадают");
    }
    ...
    public void addNewSubtask() {
        Epic epic = new Epic("Тестовый эпик 1");
        taskManager.addEpic(epic);

        Subtask subtask1 = new Subtask("Первая подзадача", TaskStatus.NEW);
    ...
    public void updateSubtasks() {
        ...
        Subtask subtask2 = new Subtask(subtask1.getEpicId(),"Третья подзадача (Upd)", TaskStatus.IN_PROGRESS);
        taskManager.updateSubtask(subtask1.getId(), subtask2);
    ...
    public void deleteSubtasks() {
        ...
        Subtask subtask1 = new Subtask("Первая подзадача", TaskStatus.NEW);
        Subtask subtask2 = new Subtask("Вторая подзадача", "Тестовая подзадача", TaskStatus.NEW);
        taskManager.addSubtask(epic.getId(), subtask1);
        taskManager.addSubtask(epic.getId(), subtask2);

        Epic epic2 = new Epic("Тестовый эпик 2");
        taskManager.addEpic(epic2);
        Subtask subtask3 = new Subtask("Третья подзадача", TaskStatus.NEW);
    ...
    public void getHistory() {
        ...
        Task taskToAdd1 = new Task("Вторая задача", "", TaskStatus.IN_PROGRESS);
        taskManager.addTask(taskToAdd1);
    ...
    public void prioritizedTasksTest() {
        ...
        Task task3 = new Task("Первая", "Моя первая задача", TaskStatus.NEW, LocalDateTime.of(2023, 9, 1, 11, 10, 0), 30);
